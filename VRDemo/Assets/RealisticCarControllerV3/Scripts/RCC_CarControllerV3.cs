...
//		anyWheel.ConfigureVehicleSubsteps(10f, 2, 2);
	...
	public IEnumerator StartEngine (){
		...
		//LogitechGSDK.LogiPlayLeds(0, 20, 20, 20);//打开 Led 灯
	...
	void DamageInit (){
		...
		//LogitechGSDK.LogiPlayFrontalCollisionForce(0, 60);//方向盘震动
	...
	public void SetPlayer(GamePlayer varPlayer)
	{
		player = varPlayer;
	}
	...
	void Inputs(){
		...
		case RCC_Settings.ControllerType.Keyboard:
			...
			case RCC_Settings.ControllerType.Custom:
				//wheelInput.InputV();//更新按键数据

				//刹车，方向盘，油门
				gasInput = wheelInput.accelerator;
				brakeInput = wheelInput.footbrake;
				steerInput = wheelInput.steeringWheel;


				//boostInput = Input.GetKey(RCCSettings.boostKB) ? 2.5f : 1f;
				handbrakeInput = wheelInput.isUseHandbrake ? 1f : 0f;//手刹
				...
				//近光灯，远光灯
				if (wheelInput.isOpenLowHeadLight)
				{
					lowBeamHeadLightsOn = !lowBeamHeadLightsOn;
				}
				if (wheelInput.isOpenHighHeadLight)
				{
					highBeamHeadLightsOn = !highBeamHeadLightsOn;
				}

				//启动/停止引擎
				if (wheelInput.isStartEngine)
					KillOrStartEngine();

				//左/右指示灯，双闪灯
				if (wheelInput.isOpenRightIndicator)
				{
					if (indicatorsOn != IndicatorsOn.Right)
						indicatorsOn = IndicatorsOn.Right;
					else
				...
				if (wheelInput.isOpenLeftIndicator)
				{
					if (indicatorsOn != IndicatorsOn.Left)
						indicatorsOn = IndicatorsOn.Left;
					else
				...
				if (wheelInput.isOpenDoubleFlash)
				{
					if (indicatorsOn != IndicatorsOn.All)
					{
						indicatorsOn = IndicatorsOn.Off;
